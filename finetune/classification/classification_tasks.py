...
class ST(ClassificationTask):
  """ST."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(ST, self).__init__(config, "ST", tokenizer, ["0", "1"])

  def _create_examples(self, lines, split):
    examples = []
    examples += self._load_glue(
          lines, split, 1, None, 0, False, len(examples), False)
    return examples
...
class CDK(ClassificationTask):
  """CDK."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(CDK, self).__init__(config, "CDK", tokenizer, ["0", "1"])
...
class CK2(ClassificationTask):
  """CK2."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(CK2, self).__init__(config, "CK2", tokenizer, ["0", "1"])
  ...
  def get_scorer(self, task, split):
    return classification_metrics.AUCScorer(task, pd.read_csv(os.path.join(self.config.raw_data_dir(self.name), split + ".tsv"),  header=None, sep='\t'), split)


class MAPK(ClassificationTask):
  """MAPK."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(MAPK, self).__init__(config, "MAPK", tokenizer, ["0", "1"])
...
class PKA(ClassificationTask):
  """PKA."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(PKA, self).__init__(config, "PKA", tokenizer, ["0", "1"])
...
class PKC(ClassificationTask):
  """PKC."""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(PKC, self).__init__(config, "PKC", tokenizer, ["0", "1"])
